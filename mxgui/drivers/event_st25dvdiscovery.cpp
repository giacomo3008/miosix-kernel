...
#error unknown orientation
    ...
    public:
        void update(bool newState)
        {
            ...
            {
                if (lastState == false)
                    callback(Event(Type, EventDirection::DOWN));
                lastState = true;
            }
            else
            {
                if (lastState == true)
                    callback(Event(Type, EventDirection::UP));
                lastState = false;
            }
        }
    ...
    static void waitForTouchOrButton()
    {
        ...
        if (!touchIntSema.reset())
            touchIntSema.timedWait(t + 20000000LL);
